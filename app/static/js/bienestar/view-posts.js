...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadPosts() {
        try {
            ...
            const response = await fetch('/posts/published');
    ...
    function renderPosts(reset = false) {
        ...
        postsToRender.forEach(post => {
            ...
            // Asignar HTML con datos del post
            postElement.innerHTML = `
                <div class="card h-100 shadow-sm border-0 rounded-3 hover-elevation">
                    ${post.cover_image ? `
                        <div class="card-img-container">
                            <img src="${post.cover_image}" class="card-img-top" alt="${post.title}">
                            <div class="card-category">${getCategoryName(post.category)}</div>
                        </div>
                    ` : `
                        <div class="card-category mt-3 ms-3">${getCategoryName(post.category)}</div>
                    `}
                    <div class="card-body">
                        <h5 class="card-title mb-2">
                            <a href="/posts/post-detail?id=${post.id}" class="stretched-link text-decoration-none text-dark">
                                ${post.title}
                            </a>
                        </h5>
                    ...
                    <div class="card-footer bg-white border-0">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <a href="/posts/post-detail?id=${post.id}" class="btn btn-sm btn-outline-primary">Leer más</a>
                        </div>
                    </div>
                </div>
            `;
