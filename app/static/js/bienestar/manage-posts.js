...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderPosts() {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            row.innerHTML = `
                <td class="title-cell">
                    <a href="/posts/post-detail?id=${post.id}" target="_blank" class="text-decoration-none">
                        ${post.title}
                    </a>
                </td>
                <td>${formattedDate}</td>
    ...
    function addActionButtonsEvents() {
        ...
        document.querySelectorAll('.edit-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                window.location.href = `/posts/post-editor?id=${postId}`;
            });
        });
    ...
    async function updatePostStatus(postId, newStatus) {
        try {
            const response = await fetch(`/api/posts/update-status/${postId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
